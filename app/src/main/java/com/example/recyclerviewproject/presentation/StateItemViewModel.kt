...
import com.example.recyclerviewproject.domain.AddStateItemUseCase
import com.example.recyclerviewproject.domain.EditStateItemUseCase
import com.example.recyclerviewproject.domain.GetStateItemUseCase
...
class StateItemViewModel:ViewModel() {
    ...
    private val _stateItem=MutableLiveData<State>()
    val stateItem:LiveData<State>
        get()=_stateItem
    ...
    fun addStateItem(inputName:String?,inputCapital:String?){
        ...
        if(fieldsValue){
                ...
                val stateItem=State(id+1,name,capital,R.drawable.russia)
